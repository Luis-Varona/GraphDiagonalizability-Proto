...
def main():
    ...
    conn = sql.connect(
        "graph_data/DiagonalizableGraphs/bipartite/ConBipart_ZeroOneNegDiag_Graphs.db",
        detect_types = sql.PARSE_DECLTYPES)
    cur = conn.cursor()
    
# %% Load data from Julia and convert to instances of 'DiagGraph'
    PyPort_diag_con_bi_graphs_orders12to14 = np.load(
    ...
    diag_con_bi_graphs_orders12to14 = [
        ...
        in PyPort_diag_con_bi_graphs_orders12to14]
    
# %% Map all graph objects to the SQL database and commit
    for (order, graphs) in enumerate(diag_con_bi_graphs_orders12to14, 12):
