...
# %% Get length of graph6 string for SQL VARCHAR type
def get_string_length(order: int) -> int:
    if order == 12:
        length = 12
    elif order == 13:
        length = 14
...
def main():
    ...
    conn = sql.connect(
        "graph_data/DiagonalizableGraphs/regular/ConRegular_ZeroOneNegDiag_Graphs.db",
        detect_types = sql.PARSE_DECLTYPES)
    cur = conn.cursor()
    
# %% Load data from Julia and convert to instances of 'DiagGraph'
    PyPort_diag_con_reg_graphs_orders12to14 = np.load(
    ...
    diag_con_reg_graphs_orders12to14 = [
        ...
        in PyPort_diag_con_reg_graphs_orders12to14]
    
# %% Map all graph objects to the SQL database and commit
    for (order, graphs) in enumerate(diag_con_reg_graphs_orders12to14, 12):
        table = f"Order{order:02d}"
        cur.execute(
            f"CREATE TABLE {table} ("
            "GraphOrder TINYINT, "
            f"graph6 VARCHAR({get_string_length(order)}), "
            "Adjacency ARRAY, "
            "Laplacian ARRAY, "
