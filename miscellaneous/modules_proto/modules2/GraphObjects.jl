module GraphObjects
    ...
    An undirected graph in Laplacian form with data on its {-1,0,1}- and {-1,1}-bandwidths.
    
    # Fields
    - `laplacian_matrix::Matrix{Int64}`: the (integer-weighted) Laplacian matrix.
    - `band_zerooneneg::Union{Float64, Int64}`: the {-1,0,1}-bandwidth of the graph.
    - `band_oneneg::Union{Float64, Int64}`: the {-1,1}-bandwidth of the graph.
    ...
    - `networkx_graph::Py`: a NetworkX instance (in Python) of the graph.
    ...
    struct DiagGraph
        ...
        band_zerooneneg::Union{Float64, Int64}
        band_oneneg::Union{Float64, Int64}
    ...
    function Base.getproperty(g::DiagGraph, prop::Symbol)
        ...
        if prop == :size
            ...
            # return convert(Int64, sum(A_unweighted)/2)
        ...
        if prop == :networkx_graph
            return nx.from_numpy_array(np.array(g.adjacency_matrix))
        end
        ...
        if prop == :is_regular
            return pyconvert(Bool, nx.is_regular(g.networkx_graph))
        end
        if prop == :is_bipartite
            return pyconvert(Bool, nx.is_bipartite(g.networkx_graph))
        end
        if prop == :is_planar
            return pyconvert(Bool, nx.is_planar(g.networkx_graph))
        end
    end
